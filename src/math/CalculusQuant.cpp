...
namespace hermesml {
    ...
    CalculusQuant::CalculusQuant(const HEContext &ctx) : EncryptedObject(ctx), constants(Constants(ctx)) {}

    Ciphertext<DCRTPoly> CalculusQuant::TaylorSqrt(const Ciphertext<DCRTPoly>& x) {
    ...
    Ciphertext<DCRTPoly> CalculusQuant::Euclidean(const Ciphertext<DCRTPoly>& point1,
