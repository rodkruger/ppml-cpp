...
int main() {
    ...
    auto normalizedData = wineData;
    scaler.Scale(normalizedData);
    auto quantizedData = Quantizer().Quantize(normalizedData);
    ...
    int64_t trainingLength = quantizedData.size() * 0.7;
    int64_t testingLength = quantizedData.size() - trainingLength;

    auto trainingData = std::vector<std::vector<int64_t>>();
    auto testingData = std::vector<std::vector<int64_t>>();
    ...
    std::copy(quantizedData.begin(), quantizedData.begin() + trainingLength, std::back_inserter(trainingData));
    std::copy(wineLabels.begin(), wineLabels.begin() + trainingLength, std::back_inserter(trainingLabels));
    std::copy(quantizedData.begin() + trainingLength, quantizedData.end(), std::back_inserter(testingData));
    ...
    auto encryptedTrainingData = bgvClient.Encrypt(trainingData);
    ...
    auto encryptedTestingData = bgvClient.Encrypt(testingData);
